	...
	int L)
{
	...
	//sxs_spf_full_write("spfa", A);
	//sxs_spf_full_write("spfb", B);
	
	double beta_bgn = 0.0;
	double beta_end = M_PI;
	double beta_step = (beta_end - beta_bgn) / (nbeta - 1);
	
	printf("num z steps = %i\n", znum);
	...
	//int* mask5d = calloc(size5d, sizeof(int));
	...
	//mpf_init(w3j1);
	//mpf_set_d(w3j1, 2.0);
	//mpf_mul(w3j1, w3j1, w3j1);
	//mpf_mul(w3j1, w3j1, w3j1);
	//
	//wigner_3j_symbol_arb(w3j1, 5, 5, 10, -1, 0, 1, fact);
	//printf("%f\n", mpf_get_d(w3j1));
	//exit(0);
	...
	/*for (int l = 0; l <= L; l++)
		for (int l1 = 0; l1 <= L; l1++)
			...
			{
				for (int p = abs(l -l1); p < l + l1; p++)
					if ((l1 + l + p) % 2 == 1 && fabs(d_symb[d_symb_index(l, m, l1, p, L)]) > 0.000000001)
						//printf("%f %f\n", d_symb[d_symb_index(l, m, l1, p, L)], d_symb[d_symb_index(l1, m, l, p, L)]);
			}
	getchar();*/
	...
	/*for (int l = 0; l < L+1; l++)
		for (int m = 0; m < l; m++)
		{
			...
			{
				printf("(%i, %i, %i): %.1e\n", l, m, m1, d_wigner[0]->data[index_d_array(L, l, m, m1)]);
			}
			getchar();
		}*/
	...
	if (SKIP_PROFILES == 1) {
		...
		int list_id;
		struct saxs_index saxs_id;
		for (int i = 0; i < nout; i++) {
		
			saxs_disassemble_index (&saxs_id, index_out[i], nbeta, L);
	...
	for (int z_idx = 0; z_idx < znum; z_idx++) { 
		...
		double zval = z_beg + z_idx * z_step;
		
		if (SKIP_PROFILES == 1) {
			if (z_skip[z_idx] != 1) {
#ifdef _MPI_
				printf("process %i: skipping z = %.2f\n", myrank, zval);	
#else			
				printf("skipping z: %.2f\n", zval);
#endif						
				continue;
			}
		}
		...
		/*printf("tmat\n");
		for (int i = 0; i < (L+1)*(L+1)*(L+1); i++)
			if (fabs(t_matrix_re[i]) > 0.0)
				printf("%i %f\n", i, t_matrix_re[i]);
		getchar();*/
...
#ifdef _MPI_
					printf("process %i: skipping z = %.2f\tbeta1 = %.2f\t\n", myrank, zval, b1_val);	
#else			
					printf("skipping beta1: %.2f\n", b1_val);
#endif						
					continue;
				}
			}
...
#ifdef _MPI_
			printf("process %i: z = %.2f\tbeta1 = %.2f\n", myrank, zval, b1_val);	
#else
			printf("beta1: %.2f\n", b1_val);
#endif
...
#ifdef _MPI_
			printf("process %i: z = %.2f\tbeta1 = %.2fbeta2 = %.2f\n", myrank, zval, b1_val, b2_val);	
#else
			printf("beta2: %.2f\n", b2_val);
#endif
...
#ifdef _MPI_
						printf("process %i: skipping z = %.2f\tbeta1 = %.2f\tbeta2 = %.2f\n", myrank, zval, b1_val, b2_val);	
#else			
						printf("skipping beta2: %.2f\n", b2_val);
#endif						
						continue;
					}
				}
				...
				//int tmptmp = 0;
				//struct saxs_index saxs_idid;
				//saxs_disassemble_index(&saxs_idid, index_out[tmptmp], nbeta, L);
				//if (z_idx != saxs_idid.z || beta1 != saxs_idid.b1 || beta2 != saxs_idid.b2) { continue; }
				...
				if (SKIP_PROFILES == 1) {
					...
					for (int i = 0; i < nout; i++) {
						tmp = index_out[i];
						/*if (tmp / size5d == z_idx) {
							if ((tmp / (fft_size * nbeta)) % nbeta == beta1) {
								if ((tmp / fft_size) % nbeta == beta2) {
									mask[tmp % fft_size] = 1;
									nprofile++;
								}
							}
						...
						saxs_disassemble_index(&saxs_id, tmp, nbeta, L);
				...
				for (int q = 0; q < qnum; q++) {
					...
					//compute_sum2(sum2 + 0*fft_size, sum_v_re[beta2], sum_v_im[beta2], d_beta1, A->V[q], q, L);
					//compute_sum2(sum2 + 1*fft_size, sum_d_re[beta2], sum_d_im[beta2], d_beta1, A->V[q], q, L);
					//compute_sum2(sum2 + 2*fft_size, sum_w_re[beta2], sum_w_im[beta2], d_beta1, A->V[q], q, L);
					
					//compute_sum2(sum2 + 3*fft_size, sum_v_re[beta2], sum_v_im[beta2], d_beta1, A->D[q], q, L);
					//compute_sum2(sum2 + 4*fft_size, sum_d_re[beta2], sum_d_im[beta2], d_beta1, A->D[q], q, L);
					//compute_sum2(sum2 + 5*fft_size, sum_w_re[beta2], sum_w_im[beta2], d_beta1, A->D[q], q, L);
					
					//compute_sum2(sum2 + 6*fft_size, sum_v_re[beta2], sum_v_im[beta2], d_beta1, A->W[q], q, L);
					//compute_sum2(sum2 + 7*fft_size, sum_d_re[beta2], sum_d_im[beta2], d_beta1, A->W[q], q, L);
					//compute_sum2(sum2 + 8*fft_size, sum_w_re[beta2], sum_w_im[beta2], d_beta1, A->W[q], q, L);
					
					compute_sum2(sum2, sum_v_re[beta2], sum_v_im[beta2], sum_d_re[beta2], sum_d_im[beta2], sum_w_re[beta2], sum_w_im[beta2], d_beta1, A, q, L);
					...
					/*if (beta1 == 5 && beta2 == 10 && q == 10) {
						FILE* f = fopen("./new_sum2", "w");
						for (int i = 7*fft_size; i < 8*fft_size; i++) {
							fprintf(f, "%.10f %.10f\n", creal(sum2[i]), cimag(sum2[i]));
						}
						fclose(f);
					}*/

					/*printf("sum2\n");
					for (int i = 0; i < 9*fft_size; i++)
						if (cabsf(sum2[i]) > 0.0)
							printf("(%i, %i, %i), %e %e\n", q, i/fft_size, i%fft_size, creal(sum2[i]), cimag(sum2[i]));
					getchar();*/
				...
				/*if (beta1 == 5 && beta2 == 10) {
					compile_intensity(profiles[2000], params, 1.0, 0.0);
					pprint_profile_dimer("./new_profile", profiles[2000]);
					exit(0);
				} else {
					continue;
				}*/
				...
				//tmptmp = (saxs_idid.a2*(2*L+1) + saxs_idid.g1)*(2*L+1) + saxs_idid.g2;
				//compile_intensity(profiles[tmptmp], params->rm, 1.0, 0.0);
				//pprint_profile_dimer("fitted_profile_fast", profiles[tmptmp]);
				//exit(0);
				
				printf("\t%.5f: (fit params)\n", (double)(clock()-time1) / CLOCKS_PER_SEC);
				time1 = clock();
				/*for (int i = 0; i < fft_size; i++) {
				...
				/*if (SAVE_CURVES == 1)
                {
                	double alpha, fi, gamma;
                	double alpha_step = 2.0 * M_PI / (2*L+1);
                  	int i_step = (fft_size - 1) / 2;
                  	
                  	if (i_step == 0) {
                  		i_step = 1;
                  	}
                  	
                    for (int i = 0; i < fft_size; i += i_step)
                    {
                    	int tmp = i;
                       	gamma = 2.0 * M_PI - (tmp % (2*L + 1)) * alpha_step;
						tmp /= 2*L + 1;
						
						fi = (tmp % (2*L + 1)) * alpha_step;
						tmp /= 2*L + 1;
						
						alpha = 2.0 * M_PI - (tmp % (2*L + 1)) * alpha_step;
                    
                        char suffix[20];
                        sprintf(suffix, "%d", counter);
                        char curvepath[100] = STORED_CURVES_PATH;

                        strcat(curvepath, suffix);
                        pprint_profile_dimer(curvepath, profiles[i]);

                        char parampath[100] = CURVES_PARAMS_PATH;
                        FILE* pfile = fopen(parampath, "a");
                        fprintf(pfile, "%d\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n", 
                        	counter, 
                        	profiles[i]->score, zval, 
                        	beta1*beta_step, fi, 
                        	alpha, beta2*beta_step, gamma, 
                        	profiles[i]->c1, 
                        	profiles[i]->c2);
                        fclose(pfile);
		...
		if (nout > 0)
		{
			...
			for (int i = 0; i < nout; i++)
			{
				...
				{
					saxs_id %= size5d;
					scores_out[i] = scores_5d[saxs_id];
					    c1_out[i] = c1_5d[saxs_id];
					    c2_out[i] = c2_5d[saxs_id];
				}
			}
		}
	}
